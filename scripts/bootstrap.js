...
const debug = createDebugger('dev')

const packagesDir = path.resolve(__dirname, '../packages')
const files = fs.readdirSync(packagesDir)

debug('bootstrap args are:', args)
...
files.forEach(shortName => {
  ...
  const name = shortName === 'mini-vue' ? shortName : `@mini-vue/${shortName}`
  ...
  if (args.force || !fs.existsSync(apiExtractorPath)) {
    const apiExtractorConfig = {
      extends: '../../api-extractor.json',
      mainEntryPointFilePath:
        './dist/packages/<unscopedPackageName>/src/index.d.ts',
      dtsRollup: {
